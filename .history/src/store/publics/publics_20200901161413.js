...
export default {
    ...
    actions: {
        ...
        async getState({ dispatch },{uId,type,pagenum, pagesize, query}) {
            try {
        ...
        async getParty({ dispatch },{username,password,email,mobile, pagenum, pagesize, query }) {
            try {
        ...
        async getDelete({ dispatch },{id, pagenum, pagesize, query }) {
            try {
                let res = await api.getDelete({
                    id
                })
                if (res.meta.status === 200) {
                    dispatch('getUserList',
                ...
                console.log(err);
                Message.success(err.meta.msg)
            }
        },
        //修改用户
        async getAmend({ dispatch },{id,email,mobile,pagenum, pagesize, query }) {
            try {
                let res = await api.getAmend({
                    id,email,mobile
                })
                if (res.meta.status === 200) {
                    dispatch('getUserList',
