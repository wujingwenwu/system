  ...
  <div>
    ...
    <template slot-scope="props">
        <el-form label-position="left" inline class="demo-table-expand">
          <el-form-item label="">
<template v-for="item in roles.row.children">
                <div :key="item.id" class="d-flex aaa">
                  <div style="margin-right:40px" class="flex-a-c">
                    <el-tag :type="item.type" closable @close="close(item.id)">{{item.authName}}</el-tag>
                    <i class="el-icon-caret-right marg-les"></i>
                  </div>
                  <div class="f-dir-c">
                    <div v-for="item1 in item.children" :key="item1.id" class="d-flex">
                      <div class="flex-a-c marg-bos" style="margin-right:40px">
                        <el-tag
                          :type="item1.type"
                          closable
                          @close="close(item1.id)"
                      ...
                      <div class="d-flex f-f-warp">
                        <div
                          ...
                          :key="item2.id"
                          class="marg-bos"
                          :class="{'marg-ris':index<item1.children.length-1}"
                        >
                          <el-tag
                            :type="item2.type"
                            closable
                            @close="close(item2.id)"
...
export default {
  ...
  components: {},
  methods: {
    ...mapActions(["getRoles", "postAddrole", "deleteRole", "editRole"]),
    //获取下标
    indexMethod(index) {
      return index * 1 + 1;
    },
